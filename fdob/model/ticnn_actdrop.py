...
class TICNN(nn.Module):
    ...
    def __init__(self, n_classes: int = 10, device: str = None, act_layer=True):
        """
        Parameters
        ----------
        n_classes: int
        ...
        self.conv_layers = nn.Sequential(
            ...
            # conv2
            Conv1d(16, 32, 3, stride=1, padding="same", norm_layer=nn.BatchNorm1d, act_layer=act_layer),
            torch.nn.MaxPool1d(2, 2),
            # conv3
            Conv1d(32, 64, 3, stride=1, padding="same", norm_layer=nn.BatchNorm1d, act_layer=act_layer),
            torch.nn.MaxPool1d(2, 2),
            # conv4
            Conv1d(64, 64, 3, stride=1, padding="same", norm_layer=nn.BatchNorm1d, act_layer=act_layer),
            torch.nn.MaxPool1d(2, 2),
            # conv5
            Conv1d(64, 64, 3, stride=1, padding="same", norm_layer=nn.BatchNorm1d, act_layer=act_layer),
            torch.nn.MaxPool1d(2, 2),
            # conv6
            Conv1d(64, 64, 3, stride=1, padding=0, norm_layer=nn.BatchNorm1d, act_layer=act_layer),
            torch.nn.MaxPool1d(2, 2),
        )

        with torch.no_grad():
            dummy = torch.rand(1, 1, 2048).to(device)
            dummy = self.first_conv.to(device)(dummy, self.dropout_rate, False)
